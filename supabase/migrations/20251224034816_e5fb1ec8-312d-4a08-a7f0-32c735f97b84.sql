-- Make user_id nullable and drop the foreign key constraint since we use credential-based auth
ALTER TABLE public.subscriptions ALTER COLUMN user_id DROP NOT NULL;

-- Drop the foreign key constraint if it exists
ALTER TABLE public.subscriptions DROP CONSTRAINT IF EXISTS subscriptions_user_id_fkey;

-- Update the credential_login function to not set user_id
...
BEGIN
  ...
  -- Create subscription (only use credential_key_id, not user_id)
